-- Plantvorm-filter (hangplant, toef, stam, …) voor admin catalogus
-- Helper + p_form op admin_catalog_search

create or replace function public.nk_matches_plant_form(
  p_variety text,
  p_description text,
  p_tags jsonb,
  p_form text
...
immutable
as $fn$
  select case lower(coalesce(p_form, ''))
    when '' then true
    when 'hang' then (
      -- geen hang-bakken / hanging baskets
      not coalesce(p_variety, '') ~* 'hanging (basket|bowl|globe)|balkonhanger|flowerpot hanger|hanging pot'
      and (
        coalesce(p_variety, '') ~* '(hangers?|hangplant|hanging|ampel|cascade|hangend)'
        or coalesce(p_description, '') ~* '(hangplant|hanger|hanging[[:space:]]+plant|ampelplant)'
        or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Hang"'
        or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Hangend"'
        or coalesce(p_tags::text, '') ~* 'Description_EN"[[:space:]]*:[[:space:]]*"Hang"'
      )
    )
    when 'toef' then (
      coalesce(p_variety, '') ~* 'toef'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Toef"'
    )
    when 'bush' then (
      coalesce(p_variety, '') ~* '(bush|struik)'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Bush'
    )
    when 'stam' then (
      coalesce(p_variety, '') ~* '(stam|op stam|multistam|gevlochten|2-stam|multi stam)'
      or (
        coalesce(p_tags::text, '') ~* 'BasicShapeTrunk'
        and coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"(Recht|Multistam|Gevlochten|Spiraal)"'
      )
    )
    when 'vertakt' then (
      coalesce(p_variety, '') ~* 'vertakt'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Vertakt'
    )
    when 'bol' then (
      (
        coalesce(p_variety, '') ~* '(^|[^a-z])bol([^a-z]|$)|bolvorm'
        and coalesce(p_variety, '') !~* 'pot bol|vaas bol|bolmos'
      )
      or coalesce(p_tags::text, '') ~* 'PlantShape[^]]*Description_NL"[[:space:]]*:[[:space:]]*"Bol"'
    )
    when 'piramide' then (
      coalesce(p_variety, '') ~* 'piramide'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Piramide"'
    )
    when 'zuil' then (
      coalesce(p_variety, '') ~* '(zuil|draadzuil|zuilvorm)'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Zuil"'
    )
    when 'bonsai' then (
      coalesce(p_variety, '') ~* 'bonsai'
      or coalesce(p_tags::text, '') ~* 'Description_NL"[[:space:]]*:[[:space:]]*"Bonsai"'
    )
    when 'klim' then (
      coalesce(p_variety, '') ~* '(klim|climbing)'
      or coalesce(p_description, '') ~* '(klimplant|climbing)'
    )
    else true
  end;
$fn$;

comment on function public.nk_matches_plant_form is
  'Match plantvorm-filter (hang/toef/bush/stam/…) op variety, description en tags.';

grant execute on function public.nk_matches_plant_form(text, text, jsonb, text) to authenticated;

-- Oude signaturen droppen (16 args zonder p_form)
drop function if exists public.admin_catalog_search(
  text, text, text, text, text, text, numeric, numeric, numeric, numeric, text, text, text, int, int, text
);

-- admin_catalog_search + plantvorm-filter (p_form):
...
begin
  ...
  filtered as (
    ...
    where
      ...
      and (
        coalesce(p_form, '') = ''
        or public.nk_matches_plant_form(
          b.item_variety_nl,
          b.description,
          b.tags,
          p_form
