-- Foto-filter: gebruik has_image wanneer gezet, anders item_picture_name.
-- Voorkomt “Met NK-foto = 0” zodra has_image massaal null is én maakt
-- “Zonder NK-foto” nuttig na check-image-availability.
...
begin
  ...
  enriched as (
    select
      ...
      -- Effectieve “heeft foto”-flag voor filter + imageItemcode
      case
        when b.has_image is true then true
        when b.has_image is false then false
        when nullif(trim(b.item_picture_name), '') is not null then true
        else false
      end as has_nk_photo
    from base b
  ),
  filtered as (
    ...
    where
      ...
      and (
        coalesce(p_photo, '') = ''
        or (p_photo = 'yes' and e.has_nk_photo is true)
        or (p_photo = 'no' and e.has_nk_photo is false)
      )
      and (
        coalesce(p_offered, '') = ''
      ...
      and (
        ...
        or (p_optimized = 'yes' and e.optimized is true)
        or (p_optimized = 'no' and e.optimized is not true)
      )
  ),
  counted as (
      ...
      (
        select jsonb_agg(
          jsonb_build_object(
            ...
            'width', p.width,
            'imageItemcode', case
              when p.has_nk_photo then p.itemcode
              else null
            end,
