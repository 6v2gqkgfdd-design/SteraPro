-- =====================================================================
-- Snellere admin_catalog_search (één CTE-query, lichte tag-handling)
-- =====================================================================
...
begin
  ...
  v_need_tags := (coalesce(p_brand, '') <> '') or (coalesce(p_location, '') <> '');
  ...
  filtered as (
    ...
    where
      ...
      and (
        coalesce(p_location, '') = ''
        or (b.tags_text is not null and b.tags_text ilike '%' || p_location || '%')
      )
      and (
        coalesce(p_stock, '') = ''
