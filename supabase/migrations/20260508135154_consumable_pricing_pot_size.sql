-- Verbruiksgoederen-prijzen + potmaat-veld op planten.
--
-- 1. consumable_catalog krijgt prijs-velden zodat de werkbon
--    automatisch een totaal kan tonen. Prijzen in cents (integer)
--    om afrondingsproblemen te vermijden.
-- 2. plants krijgen een pot_size_code (bv. 'C3', 'C4') zodat we
--    bij verpotting weten wat de huidige maat is en automatisch
--    één maat groter kunnen voorstellen.
-- 3. Seed de catalog met de prijzen + standaard hoeveelheden die
--    Jelle hanteert (zie chat).

-- ── 1. Prijs-velden op consumable_catalog ──────────────────────
alter table public.consumable_catalog
  add column if not exists unit_size numeric(10,2),       -- bv. 10 (liter per zak), 1 (per stuk)
  add column if not exists unit_price_cents integer,      -- prijs per unit_size
  add column if not exists default_quantity numeric(10,2), -- standaard quantity bij toevoegen
  add column if not exists description text;

-- ── 2. Potmaat-code op plants ──────────────────────────────────
alter table public.plants
  add column if not exists pot_size_code text;

create index if not exists plants_pot_size_code_idx
  on public.plants(pot_size_code)
  where pot_size_code is not null;

-- ── 3. Seed/update catalog met prijzen ─────────────────────────
-- Bestaande items bijwerken (insert with on conflict werkt niet voor
-- meerdere kolommen update — gebruik UPDATE).
...
update public.consumable_catalog
   set default_unit = 'L',
       ...
       default_quantity = 10,
       description = '10 liter zak'
 where name = 'Hydrokorrels';

update public.consumable_catalog
   set name = 'Voeding (Pokon)',
       default_unit = 'flesje',
       unit_size = 1,
       unit_price_cents = 1600,
       default_quantity = 1,
       description = 'Pokon flesje per onderhoud'
 where name = 'Voeding (Pokon)';

update public.consumable_catalog
   set default_unit = 'spuitbus',
       unit_size = 1,
       unit_price_cents = 1200,
       default_quantity = 1,
       description = 'Bladglans-spuitbus, ± 1 per onderhoud'
 where name = 'Bladglans';

-- ── 4. Werkbon-RPC bijwerken: prijs-info doorgeven aan signing-pagina
    ...
    'consumables', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'unit', vc.unit,
          'notes', vc.notes,
          'unit_size', cc.unit_size,
          'unit_price_cents', cc.unit_price_cents
        )
