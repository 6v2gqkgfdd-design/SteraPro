-- RPC get_work_order_for_signing uitbreiden met de behandelde
-- ruimtes van de beurt. Wordt op de klant-link (sign/[token])
-- als pillen onder de locatie getoond.
...
begin
  select jsonb_build_object(
    ...
    'rooms', coalesce((
      ...
      from public.maintenance_visit_rooms mvr
      join public.rooms r on r.id = mvr.room_id
      where mvr.visit_id = v.id
    ), '[]'::jsonb),
    'plants', coalesce((
      select jsonb_agg(
        jsonb_build_object(
