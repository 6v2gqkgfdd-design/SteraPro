-- =====================================================================
-- admin_catalog_search: effectieve locatie (NK + product_enrichment)
-- =====================================================================
...
begin
  ...
  -- Brand-filter nog via tags-tekst; locatie via nk_location_from_tags + enrichment
  ...
  with base as (
    select
      ...
      -- Locatie: NK eerst, anders Stera-enrichment
  ...
  filtered as (
    ...
    where
      ...
      and (
        ...
        or (
          p_location = 'missing'
          and cardinality(coalesce(b.effective_locations, '{}'::text[])) = 0
        )
        or (
          p_location in ('Binnen', 'Buiten')
          and p_location = any (coalesce(b.effective_locations, '{}'::text[]))
        )
      )
      and (
        coalesce(p_stock, '') = ''
      ...
      and (
        coalesce(p_brand, '') = ''
        or (b.tags_text is not null and b.tags_text ilike '%' || p_brand || '%')
      )
      and (
